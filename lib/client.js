...
.dcc-loghead { display: flex; align-items: center; justify-content: space-between; }
.dcc-loghead .dcc-contacts-title { margin: 12px 0 4px; }
.dcc-loghead button { width: auto; margin: 12px 0 0; padding: 4px 12px; font-size: 11px; border-radius: 8px; }
...
`;
		...
		function apply(ctx) {
			...
			const clearLogBtn = el("button", { text: "清空记录" });
			...
			const logHead = el("div", { class: "dcc-loghead" });
			logHead.append(logTitle, clearLogBtn);
			const log = el("ul", { class: "dcc-log" });
			panelBody.append(contactsTitle, contacts, testBtn, muteBtn, logHead, log);

...
.dcc-boss { position: fixed; right: 18px; bottom: 18px; z-index: 2147483000; width: 96px; height: 96px; border-radius: 50%; cursor: pointer; border: 2px solid #c5a468; box-shadow: 0 10px 30px #0f1e4833; overflow: hidden; background: #0b1942; animation: dcc-bob 3.2s ease-in-out infinite; }
.dcc-boss img { width: 100%; height: 100%; object-fit: cover; object-position: 50% 12%; display: block; }
.dcc-boss::after { content: ""; position: absolute; inset: -6px; border-radius: 50%; border: 2px solid #c5a46866; animation: dcc-pulse 2.4s ease-out infinite; }
.dcc-boss .dcc-badge { position: absolute; right: -2px; top: -2px; min-width: 22px; height: 22px; border-radius: 11px; background: #c0392b; color: #fff; font-size: 12px; line-height: 22px; text-align: center; padding: 0 6px; font-family: system-ui; display: none; }
@keyframes dcc-bob { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-6px); } }
@keyframes dcc-pulse { 0% { transform: scale(1); opacity: .7; } 100% { transform: scale(1.25); opacity: 0; } }
.dcc-panel { position: fixed; right: 18px; bottom: 124px; z-index: 2147483000; width: 320px; max-height: 60vh; overflow: auto; background: #f8f6f0f2; backdrop-filter: blur(8px); border: 1px solid #c5a46888; border-radius: 14px; padding: 12px; font-family: system-ui, "Microsoft YaHei", sans-serif; color: #172347; box-shadow: 0 18px 54px #0f1e4840; display: none; }
.dcc-panel.open { display: block; }
.dcc-panel h4 { margin: 0 0 8px; font-size: 14px; color: #10204d; }
.dcc-panel button { display: block; width: 100%; margin: 6px 0; padding: 8px 10px; border-radius: 8px; border: 1px solid #536eae88; background: #fffdf8; color: #172347; cursor: pointer; font-size: 13px; font-family: inherit; }
.dcc-panel button:hover { background: #ece6d8; }
.dcc-log { list-style: none; margin: 8px 0 0; padding: 0; font-size: 12px; }
.dcc-log li { padding: 4px 0; border-top: 1px dashed #c5a46855; }
...
.dcc-wx-card { position: relative; width: 324px; max-width: 92vw; background: #ffffff; border-radius: 20px; padding: 28px 20px 22px; text-align: center; font-family: system-ui, "Microsoft YaHei", sans-serif; color: #191919; box-shadow: 0 24px 80px #00000055; }
.dcc-wx-card .dcc-wx-avatar { width: 92px; height: 92px; border-radius: 50%; object-fit: cover; object-position: 50% 12%; margin: 0 auto 12px; display: block; background: #e7e7e7; box-shadow: 0 8px 24px #00000022; }
.dcc-wx-card .dcc-wx-name { font-size: 17px; font-weight: 600; }
.dcc-wx-card .dcc-wx-sub { font-size: 13px; color: #888888; margin: 6px 0 16px; }
...
.dcc-wx-btn { width: 58px; height: 58px; border-radius: 50%; border: none; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; color: #fff; box-shadow: 0 6px 16px #00000022; transition: transform .12s ease; }
.dcc-wx-btn:hover { transform: scale(1.07); }
.dcc-wx-btn.decline { background: #fa5151; }
.dcc-wx-btn.accept { background: #07c160; animation: dcc-wx-pulse 1.2s ease-in-out infinite; }
@keyframes dcc-wx-pulse { 0%,100% { box-shadow: 0 6px 16px #00000022, 0 0 0 0 #07c16066; } 50% { box-shadow: 0 6px 16px #00000022, 0 0 0 16px #07c16000; } }
.dcc-wx-bubble { display: none; margin: 14px 0 0; background: #f5f5f5; border-radius: 12px; padding: 10px 12px; font-size: 13px; line-height: 1.6; text-align: left; color: #333333; white-space: pre-wrap; max-height: 170px; overflow: auto; }
.dcc-wx-toolrow { display: flex; gap: 8px; justify-content: center; margin-top: 12px; flex-wrap: wrap; }
.dcc-wx-toolrow button { padding: 6px 14px; border-radius: 999px; border: 1px solid #07c16088; background: #ffffff; color: #07c160; cursor: pointer; font-size: 12.5px; font-family: inherit; }
.dcc-wx-toolrow button:hover { background: #07c16010; }
.dcc-wx-ringbtn { display: none; width: 100%; margin: 10px 0 4px; padding: 10px 12px; border-radius: 12px; border: none; background: #07c160; color: #ffffff; font-size: 14px; font-weight: 600; cursor: pointer; font-family: inherit; }
.dcc-wx-ringbtn:hover { background: #06a854; }
.dcc-wx-typing { margin-top: 6px; font-size: 12px; color: #888888; font-style: italic; animation: dcc-wx-blink 1.2s ease-in-out infinite; }
@keyframes dcc-wx-blink { 0%,100% { opacity: 1; } 50% { opacity: .35; } }
.dcc-contacts-title { font-size: 13px; color: #10204d; margin: 10px 0 4px; font-weight: 600; }
.dcc-contact { display: flex; align-items: center; gap: 8px; width: 100%; padding: 6px 8px; margin: 4px 0; border-radius: 10px; border: 1px solid #536eaa33; background: #fffdf8; cursor: pointer; font-family: inherit; text-align: left; }
.dcc-contact:hover { background: #ece6d8; }
.dcc-contact img { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; object-position: 50% 12%; flex: none; }
.dcc-contact-meta { text-align: left; min-width: 0; }
.dcc-contact-name { font-size: 13px; color: #172347; }
.dcc-contact-role { font-size: 11px; color: #6f7c99; }
.dcc-contacts-empty { font-size: 12px; color: #6f7c99; padding: 4px 2px; }
.dcc-contact { position: relative; }
.dcc-contact-voice { font-size: 10.5px; color: #8a94aa; }
.dcc-contact-voicebtn { flex: none; padding: 4px 8px; border-radius: 999px; border: 1px solid #536eaa66; background: #fffdf8; color: #536eae; font-size: 11px; cursor: pointer; margin-left: auto; font-family: inherit; }
.dcc-contact-voicebtn:hover { background: #ece6d8; }
.dcc-voice-mask { position: fixed; inset: 0; z-index: 2147483002; background: #09133366; display: flex; align-items: center; justify-content: center; }
.dcc-voice-box { width: 400px; max-width: 94vw; max-height: 78vh; overflow: auto; background: #fffdf8; border: 2px solid #c5a468; border-radius: 14px; padding: 14px; font-family: system-ui, "Microsoft YaHei", sans-serif; color: #172347; text-align: left; box-shadow: 0 24px 80px #00000055; }
.dcc-voice-box h4 { margin: 0 0 8px; text-align: center; }
.dcc-voice-filter { display: flex; gap: 6px; justify-content: center; margin-bottom: 8px; }
.dcc-voice-filter button { padding: 4px 12px; border-radius: 999px; border: 1px solid #536eaa88; background: #fff; color: #536eae; cursor: pointer; font-size: 12px; font-family: inherit; }
.dcc-voice-filter button.on { background: #536eae; color: #fff; }
.dcc-voice-sec { font-size: 12px; font-weight: 600; color: #10204d; margin: 10px 0 4px; }
.dcc-voice-row { display: flex; align-items: center; gap: 6px; padding: 5px 4px; border-top: 1px dashed #c5a46844; font-size: 12px; }
.dcc-voice-row .vname { flex: 1; min-width: 0; font-weight: 600; }
.dcc-voice-row .vstyle { color: #6f7c99; font-size: 11px; flex: none; max-width: 40%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.dcc-voice-row button { flex: none; padding: 3px 10px; border-radius: 999px; border: 1px solid #07c16088; background: #fff; color: #07c160; cursor: pointer; font-size: 11px; font-family: inherit; }
.dcc-voice-row button.cur { background: #07c160; color: #fff; }
.dcc-voice-close { display: block; margin: 12px auto 0; padding: 6px 18px; border-radius: 999px; border: 1px solid #536eaa88; background: #fff; color: #536eae; cursor: pointer; font-family: inherit; }
.dcc-wx-inputrow { display: none; margin-top: 8px; gap: 6px; }
.dcc-wx-inputrow input { flex: 1; min-width: 0; padding: 8px 12px; border-radius: 999px; border: 1px solid #d0d0d0; font-size: 13px; font-family: inherit; outline: none; }
.dcc-wx-inputrow input:focus { border-color: #07c160; }
.dcc-wx-inputrow button { flex: none; padding: 8px 16px; border-radius: 999px; border: none; background: #07c160; color: #ffffff; font-size: 13px; cursor: pointer; font-family: inherit; }
.dcc-wx-inputrow button:hover { background: #06a854; }
.dcc-ask { position: fixed; inset: 0; z-index: 2147483002; background: #09133366; display: none; align-items: center; justify-content: center; }
.dcc-ask.open { display: flex; }
.dcc-ask-box { width: 320px; max-width: 92vw; background: #fffdf8; border: 2px solid #c5a468; border-radius: 14px; padding: 18px; font-family: system-ui, "Microsoft YaHei", sans-serif; color: #172347; text-align: center; box-shadow: 0 24px 80px #00000055; }
.dcc-ask-box p { margin: 8px 0 14px; font-size: 14px; }
.dcc-ask-box .dcc-actions { display: flex; gap: 10px; justify-content: center; }
.dcc-ask-box button { padding: 8px 16px; border-radius: 999px; border: 1px solid #536eaa; background: #fffdf8; color: #172347; cursor: pointer; font-size: 13px; font-family: inherit; }
.dcc-ask-box button.primary { background: #536eae; color: #fff; }
.dcc-ask-box input { width: 100%; margin: 8px 0; padding: 8px 10px; border-radius: 8px; border: 1px solid #536eaa88; font-size: 13px; font-family: inherit; }
`;
		...
		function apply(ctx) {
			...
			const panelTitle = el("h4", { text: "📞 队长来电" });
			const contactsTitle = el("div", { class: "dcc-contacts-title", text: "👥 通讯录（点击队员即可拨打）" });
			...
			panel.append(panelTitle, contactsTitle, contacts, testBtn, muteBtn, log);
			...
			const renderContacts = () => {
				...
				for (const [name, info] of seenMembers) {
					...
					row.append(img, meta);
					const voiceBtn = el("button", { class: "dcc-contact-voicebtn", text: "🎙️ 声线" });
			...
			function openVoicePicker(name) {
				...
				const title = el("h4", { text: `${name} 的声线选择` });
				...
				const renderRows = () => {
					...
					const kokoro = voiceCatalog.kokoro.filter((v) => voiceFilter === "all" || v.gender === voiceFilter);
					const edge = voiceCatalog.edge.filter((v) => voiceFilter === "all" || v.gender === voiceFilter);
					const appendSection = (label, list) => {
						...
						for (const v of list) {
							...
							listen.addEventListener("click", () => playPreview(v.id));
					...
					appendSection("🌐 本地开源 · Kokoro（离线，102 款）", kokoro);
					appendSection("☁️ 微软在线 · Edge", edge);
					if (!kokoro.length && !edge.length) {
						rowsWrap.append(el("div", { class: "dcc-contacts-empty", text: "（暂无可用声线：Kokoro 模型请运行 scripts/download-models.ps1；Edge 需要网络）" }));
					}
				};
				const reset = el("button", { class: "dcc-voice-close", text: "恢复默认声线" });
				...
				renderRows();
				box.append(title, filter, rowsWrap, reset, close);
				mask.append(box);
				root.append(mask);
			}
			...
			muteBtn.addEventListener("click", () => {
				muted = !muted;
				muteBtn.textContent = `🔇 静音开关：${muted ? "开" : "关"}`;
				if (muted) stopSpeak();
			});

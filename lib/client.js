...
`;
		...
		function apply(ctx) {
			...
			function speakerFor() {
				...
				if (!member || member === (cfg?.bossName ?? "Daisy")) return cfg?.voiceMap?.ginka ?? "default";
				return cfg?.voiceMap?.[member] ?? cfg?.voiceMap?.default ?? "default";
			}
			...
			async function speak(text, onEnd) {
				...
				try {
					const response = await fetch(`/plugins/dsh-captain-call/tts?text=${encodeURIComponent(content)}&speaker=${encodeURIComponent(speakerFor())}`, { cache: "no-store" });
					if (!response.ok) throw new Error("tts http " + response.status);
					const blob = await response.blob();
					if (!blob.size) throw new Error("empty audio");
					const url = URL.createObjectURL(blob);
					ttsAudio = new Audio(url);
					ttsAudio.onended = () => { URL.revokeObjectURL(url); ttsAudio = null; onEnd?.(); };
					ttsAudio.onerror = () => { URL.revokeObjectURL(url); ttsAudio = null; speakWithSystem(content, onEnd); };
					ttsAudio.play().catch(() => { speakWithSystem(content, onEnd); });
				} catch {
					speakWithSystem(content, onEnd);
				}
			}
			...
			const renderContacts = () => {
				...
				for (const [name, info] of seenMembers) {
					const row = el("button", { class: "dcc-contact" });
					const img = el("img", { src: ASSET_URL + "/" + (avatarFor(name) || "placeholder.svg"), alt: name });
					const meta = el("div", { class: "dcc-contact-meta" });
					meta.append(el("div", { class: "dcc-contact-name", text: name }), el("div", { class: "dcc-contact-role", text: info.role ?? "" }));
					row.append(img, meta);
			...
			/* ---------- 麦克风授权（每次申请前先问） ---------- */
			function askMicPermission(reason, onAllow) {
				const ask = el("div", { class: "dcc-ask open" });
				const box = el("div", { class: "dcc-ask-box" });
				const title = el("h4", { text: "🎤 麦克风使用申请" });
				const line = el("p", { text: `ginka 想使用麦克风：${reason}。是否允许？` });
				const actions = el("div", { class: "dcc-actions" });
				const allow = el("button", { class: "primary", text: "允许" });
				const deny = el("button", { text: "拒绝" });
				const close = () => ask.remove();
				allow.addEventListener("click", () => { close(); onAllow(); });
			...
			function startRecognition() {
				...
				recognition.onend = () => {
					/* 通话未结束且麦克风仍授权 → 继续听下一句 */
					if (micStream && currentCall && !currentCall.ended) {
				...
				try { recognition.start(); showBubble("（已接通，请说话…）", true); } catch { stopMic(); }
			}

			function requestMic(reason) {
				askMicPermission(reason, () => {
			...
			function buildCallCard(data) {
				...
				toolrow.append(voiceReply, endCall);
				...
				return { mask, bubble, sub, ringBtn, accept, decline, cancel, voiceReply, endCall, inputRow, input, sendBtn };
			}

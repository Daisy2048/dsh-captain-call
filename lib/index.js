    ...
    const registerWebSurface = () => {
        ...
        ctx.effect(() => webServer.register({
            ...
            handler: async (req, res) => {
                try {
                    ...
                    res.end(JSON.stringify({ config: cfg, serverTime: Date.now(), tts: { engine: ttsState.engine, error: ttsState.initError }, teams }));
                } catch (error) {
                    res.writeHead(500);
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    if (captain && memberRec.id) {
                        ...
                        }], { source: { kind: 'plugin', plugin: 'dsh-captain-call' } }).catch(() => {});
                    }
        ...
        ctx.effect(() => webServer.register({
            ...
            handler: async (req, res) => {
                ...
                try {
                    if (ttsState.ready && cfg.ttsEngine !== 'edge') {
                        try {
                            const voiceFile = fileURLToPath(new URL(`../models/kokoro-zh/voices/${speaker}.bin`, import.meta.url));
                    ...
                    const edgeVoice = cfg.voiceMapEdge?.[speaker] ?? cfg.voiceMapEdge?.default ?? 'zh-CN-XiaoxiaoNeural';
